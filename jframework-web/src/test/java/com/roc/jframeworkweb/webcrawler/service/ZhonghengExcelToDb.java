...
public class ZhonghengExcelToDb {
    ...
    @Test
    public void test(){
        XSSFWorkbook workbook = ExcelUtils.read("d:/zhongheng-1.xlsx");
        ...
        for(Iterator<Row> iter = sheet.rowIterator(); iter.hasNext(); ){
            ...
            Integer words = new Double(row.getCell(5).getNumericCellValue()).intValue();
            ...
            WebNovel novel = new WebNovel.Builder()
                    .brief(brief.length() < 255 ? brief : brief.substring(0, 255))
                    .author(author)
                    .alias("")
            ...
            this.webNovelService.save(novel, new String[]{"2"}, "2");
        }
    }
}
